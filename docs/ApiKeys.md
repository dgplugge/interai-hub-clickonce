...
The InterAI Hub talks to four AI providers. Each one needs its own API key (a long random string you paste into the setup script). This page tells you exactly which buttons to click on each provider's website.
...
| **OpenAI** (GPT-4o, o3-mini) | Limited | **Yes** — must add a payment method | Pennies per session |
...
7. Paste the key into the setup script when it asks for the Anthropic key.
...
8. Paste the key into the setup script when it asks for the OpenAI key.
...
6. Paste the key into the setup script when it asks for the Gemini key.
...
7. Paste the key into the setup script when it asks for the Mistral key.
...
If you suspect a key has leaked, **revoke it** at the provider's dashboard immediately and create a new one. Then re-run the setup script (it will overwrite the old config after asking for confirmation, and saves a timestamped backup of the previous version just in case).
